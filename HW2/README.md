...
```bash
grep -v "^#" 1.gtf| awk '$1=="XI" && $3=="CDS"' | sort -k 5 -n | tail
...
```bash
grep -v "^#" 1.gtf | awk '$1!="IV" && $7=="-" && $3=="CDS" {print $5-$4+1}' | sort -rn | head -2
# 14730
# 12276
```

4. 寻找 XV 号染色体上长度最长的5条基因，并输出基因 id 及对应的长度。
* `awk`可以用`print $var1,$var2`，将输出结果放在多列中
* `awk`中定义的局部变量不需要用`$`号指定
```bash
grep -v "^#" 1.gtf| awk '$1=="XV" && $3=="gene" {split($10,x,";");gene_id=x[1];gsub("\"","",gene_id);print gene_id,$5-$
4+1}' | sort -rnk 2| head -5
# YOL081W 9240
# YOR396W 5391
# YOR343W-B 5314
# YOR192C-B 5314
...
5. 统计1.gtf列数
* `awk`中内建变量`NF`指定了一行的字段数目，该数目会随行变化，因为数据并不一定是结构化的数据
```bash
# 统计最大列数
grep -v "^#" 1.gtf | awk '{print NF}' | sort -rn | head -1
# 34
```